  ...
  <div class="m-4">
    <div>
      ...
      <input
        ...
        type="date"
        class="mx-2 border-b-2 border-black"
      />
    </div>
    <div v-if="balances.erc20 !== null || pending">
      <div v-if="!pending">
        <li v-for="token in balances.erc20" :key="token.TokenAddress">
          {{ token.amount }} {{ token.TokenName }}
        </li>
      </div>
...
class EthDater {
  ...
  async getDate(date, after = true, refresh = false) {
    ...
    let predictedBlock = await this.getBlockWrapper(
      Math.ceil(
        date.diff(moment.unix(this.firstBlock.timestamp), "seconds") / this.blockTime
      )
    );
...
const date = ref("");
const block = ref({ block: 0 });
const balances = ref({ erc20: null });
...
watch(date, async (newDate) => {
  pending.value = true;
  block.value = await dater.getDate(Date.parse(newDate));
  const { result } = await $fetch(
    "/api" +
      "?module=account" +
      "&action=addresstokenbalance" +
      `&address=${address.value}` +
      `&blockno=${block.value.block}`,
    {
      baseURL: config.public.apiBase,
    }
  );
  balances.value.erc20 = result.map((token) => {
    return {
      ...token,
      amount:
        parseFloat(token.TokenQuantity) *
